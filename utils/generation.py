...
from .data.cnndm import load_cnndm
...
def load_model(args):
    ...
    if args.is_seq2seq_model:
        ...
        if args.load_fp16:
            model = AutoModelForCausalLM.from_pretrained(
                args.model_name_or_path, torch_dtype=torch.float16, device_map="auto" #, token=access_token
            )
        else:
    ...
    if "llama" in args.model_name_or_path:
        tokenizer = LlamaTokenizer.from_pretrained(
            args.model_name_or_path, padding_side=padding_side # , token=access_token
        )
...
def load_hf_dataset(args):
    ...
    if dataset_name == "lfqa":
        ...
        dataset = load_cnndm(args)
        args.__dict__.update(
            {
                "truncate_input_for_prompt": False,
                "input_col_name": "article",
                "ref_output_col_name": "highlights",
            }
        )
        # other args set within the load_wikitext function
...
def generate(
    ...
    with torch.no_grad():
        ...
        output_without_watermark = generate_without_watermark(input_ids=input_ids, return_dict_in_generate=True, output_scores = True)
        print(output_without_watermark)
        print(output_without_watermark.scores)
        if args.generation_seed is not None:
            torch.manual_seed(args.generation_seed)
        output_with_watermark = generate_with_watermark(input_ids=input_ids, return_dict_in_generate=True, output_scores = True)

    if args.is_decoder_only_model:
        # need to isolate the newly generated tokens
        output_without_watermark.sequences = output_without_watermark.sequences[:, input_ids.shape[-1] :]
        output_with_watermark.sequences = output_with_watermark.sequences[:, input_ids.shape[-1] :]
